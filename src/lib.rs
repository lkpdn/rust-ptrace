...
bitflags! {
  flags Options: u32 {
    const SYS_GOOD = 1,
    const TRACE_FORK = 1 << 1,
    const TRACE_V_FORK = 1 << 2,
    const TRACE_CLONE = 1 << 3,
    const TRACE_EXEC = 1 << 4,
    const TRACE_V_FORK_DONE = 1 << 5,
    const TRACE_EXIT = 1 << 6,
    const TRACE_SECCOMP = 1 << 7,
    const EXIT_KILL = 1 << 20
  }
}
...
impl Writer {
    ...
    pub fn write_object<T: Sized>(&self, _: Address, data: &T) -> Result<(), usize> {
    ...
    pub fn write_data(&self, address: Address, buf: &Vec<u8>) -> Result<(), usize> {
        ...
        if max_addr > align_end {
            ...
            for word_idx in 0..(max_addr - align_end) as usize {
...
pub fn test_get_byte() {
    ...
    assert_eq!(get_byte(0xffffffffffff, 5), 0xff);
